...
fn build_tree() -> VNode {
    h(
        ...
        vec![
            h("header", Props::new().set("class", "header"), vec![
                h("h1", Props::new().set("class", "title"), vec![]),
            ]),
            h("div", Props::new().set("class", "card"), vec![
...
fn dump_init_preview_png() {
    ...
    let png = velox_renderer::render_vnode_to_raster_png(&styled, &sheet, 600, 400)
        .expect("png");
