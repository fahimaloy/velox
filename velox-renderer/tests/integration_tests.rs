...
#[cfg(test)]
mod tests {
    use velox_style::{Stylesheet, ComputedStyle, Display, Length, Color};
    use velox_renderer::event_binding::{EventBinder, EventType, EventData};
...
"#;
    ...
    fn test_event_binding_workflow() {
        ...
        binder.on_click("btn1", Rc::new(RefCell::new(move |_: &EventData| {
    ...
    fn test_input_event_handling() {
        ...
        binder.on_input("text-input", Rc::new(RefCell::new(move |event: &EventData| {
            if let Some(val) = &event.value {
                *value_clone.borrow_mut() = val.clone();
            }
        })));
        
        let event = EventData::new(EventType::Input, "text-input")
            .with_value("hello");
    ...
    fn test_hover_event() {
        ...
        binder.on_hover("item", Rc::new(RefCell::new(move |_: &EventData| {
    ...
    fn test_multiple_handlers_same_target() {
        ...
        binder.on_click("btn", Rc::new(RefCell::new(move |_event: &EventData| {
            *c1.borrow_mut() += 1;
        })));
    ...
    fn test_flexbox_properties() {
        ...
        style.apply_inline_style("display: flex; flex-direction: column; justify-content: center; gap: 10px;");
    ...
    fn test_typography_properties() {
        ...
        style.apply_inline_style("font-size: 18px; font-weight: bold; line-height: 1.5; text-align: center;");
    ...
    fn test_event_data_with_position() {
        let event = EventData::new(EventType::Click, "element")
            .with_position(100.5, 50.3);
    ...
    fn test_event_data_with_key() {
        let event = EventData::new(EventType::KeyPress, "input")
            .with_key(13); // Enter key
