...
fn main() {
    ...
    let make_view = {
        ...
        move |_w: u32, _h: u32| -> (VNode, Stylesheet) {
            let vnode = app::render_with_state(state.clone(), |name| match name {
    ...
    let get_title = {
        let state = state_ref.clone();
        move || state.title.borrow().to_string()
    };

    velox_renderer::run_window_vnode_skia("Velox VX Demo", make_view, on_event, get_title);
}
