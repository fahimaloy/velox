    ...
    pub fn parse_imports(&mut self, script_content: &str) {
        for line in script_content.lines() {
            ...
            if let Some(rest) = line.strip_prefix("import ") {
                if let Some((import_part, source)) = rest.split_once(" from ") {
            ...
            if let Some(rest) = line.strip_prefix("import {") {
                if let Some((names, source)) = rest.split_once("} from ") {
                    let source = source.trim().trim_matches('"').trim_matches('\'');
...
mod tests {
    ...
    fn test_resolve_path() {
        ...
        assert_eq!(relative, PathBuf::from("/home/project/src/components/Button.vx"));
