...
/// Parses the template AST and checks for:
/// 1. Resolved component elements (data-velox-component marker) with @event attrs
/// 2. Unresolved component tags (PascalCase or kebab-case) with @event attrs
fn template_needs_emit_infra(sfc: &Sfc) -> bool {
    let template = match &sfc.template {
        Some(t) => &t.content,
        None => return false,
    };
    let nodes = match crate::template_parse::parse_template_to_ast(template) {
        Ok(n) => n,
        Err(_) => return false,
    };

    fn is_likely_component_tag(tag: &str) -> bool {
        // Vue convention: PascalCase tags or kebab-case with uppercase segments
        // are treated as components. Also check for tags starting with uppercase.
        tag.chars().next().is_some_and(|c| c.is_ascii_uppercase())
            || tag.contains('-')
    }

    fn has_component_events(nodes: &[crate::template_ast::Node]) -> bool {
        for node in nodes {
            if let crate::template_ast::Node::Element { tag, attrs, children, .. } = node {
                let is_resolved_component = attrs.iter().any(|a| {
                    a.name == "data-velox-component" && a.kind == crate::template_ast::AttrKind::Static
                });
                let is_unresolved_component = is_likely_component_tag(tag);
                let has_on_events = attrs.iter().any(|a| a.kind == crate::template_ast::AttrKind::On);

                if (is_resolved_component || is_unresolved_component) && has_on_events {
                    return true;
                }
                if has_component_events(children) {
                    return true;
                }
            }
        }
